...
| Cross-role Case-list behavior | Legacy Administrator, Secretary, Prosecutor, and Process Server list routes/templates plus project-owner Acceptance Remediation Slice 6 decision | Shared visible legacy projection; six rows per page; Docket Number descending by default; role scopes and legacy commands preserved; search and sorting use displayed values; PIN concealed | Approved |
